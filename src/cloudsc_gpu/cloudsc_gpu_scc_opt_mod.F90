...
MODULE CLOUDSC_GPU_SCC_OPT_MOD

CONTAINS
  SUBROUTINE CLOUDSC_SCC_OPT(KIDIA,KLON,KLEV,NGPBLKS,PTSPHY,PT,PQ,TENDENCY_TMP_T,TENDENCY_TMP_Q,TENDENCY_TMP_A,  &
  ...
  & PFCQSNG,PFSQLTUR,PFSQITUR,PFPLSL,PFPLSN,PFHPSL,PFHPSN,TMP1,TMP2,YRECLDP,  &
  & NPROMA,NGPTOT)
    !---input
    !---prognostic fields
    ...
    REAL(KIND=JPRB),INTENT(IN) :: PNICE(:,:,:)
    ! ice number concentration (cf. CCN)

    INTEGER(KIND=JPIM),INTENT(IN) :: KLON ! Number of grid points
    INTEGER(KIND=JPIM),INTENT(IN) :: KLEV,NGPBLKS ! Number of levels
    ...
    REAL(KIND=JPRB) :: TENDENCY_LOC_T_,TENDENCY_LOC_A_,TENDENCY_LOC_Q_
    ...
    REAL(KIND=JPRB),INTENT(OUT) :: PFHPSN(:,:,:)
    REAL(KIND=JPRB),INTENT(INOUT) :: TMP1(:,:,:,:)
    REAL(KIND=JPRB),INTENT(INOUT) :: TMP2(:,:,:,:)
    ...
    REAL(KIND=JPRB) :: ZLCOND1,ZLCOND2,ZLEVAP,ZLEROS,ZRAINAUT,ZSNOWAUT,ZLIQCLD,ZICECLD
    ...
    REAL(KIND=JPRB) :: ZTP1,ZTP1_PREV
    REAL(KIND=JPRB) :: ZLDEFR
    REAL(KIND=JPRB) :: ZLDIFDT
    REAL(KIND=JPRB) :: ZLCUST
    ...
    REAL(KIND=JPRB) :: ZDA,ZLI,ZA,ZA_PREV,ZAORIG
    ...
    REAL(KIND=JPRB) :: ZQX0(NCLV-1) ! water variables at start of scheme
    REAL(KIND=JPRB) :: ZQXN(NCLV) ! new values for zqx at time+1
    REAL(KIND=JPRB) :: ZQXFG(NCLV) ! first guess values including precip
    REAL(KIND=JPRB) :: ZQXNM1(NCLV-1) ! new values for zqx at time+1 at level above
    REAL(KIND=JPRB) :: ZFLUXQ
    ...
    REAL(KIND=JPRB) :: ZPFPLSX(NCLV-1) ! generalized precipitation flux
    REAL(KIND=JPRB) :: ZLNEG(NCLV-1)
    REAL(KIND=JPRB) :: ZMELTMAX
    REAL(KIND=JPRB) :: ZFRZMAX
    REAL(KIND=JPRB) :: ZICETOT

    REAL(KIND=JPRB) :: ZQXN2D(NCLV-1)
    ...
    REAL(KIND=JPRB) :: ZFOEEW,ZFOEELIQT,ZFOEEWMT
    ...
    REAL(KIND=JPRB) :: ZEVAPLIMMIX,ZEVAPLIMICE
    ...
    REAL(KIND=JPRB) :: ZEXPLICIT,ZSINKSUM
    ...
    REAL(KIND=JPRB),PARAMETER :: ZEPSILON=100._JPRB*EPSILON(ZEPSILON)

    REAL(KIND=JPRB) :: ZCOND1,ZQP

    REAL(KIND=JPRB) :: PSUM_SOLQA
    INTEGER(KIND=JPIM) :: JKGLO,IBL,ICEND,NPROMA,NGPTOT
...
!$acc data &
!$acc present( &
!$acc   pt,pq,tendency_tmp_t,tendency_tmp_q,tendency_tmp_a,tendency_tmp_cld,&
!$acc   tendency_loc_t,tendency_loc_q,tendency_loc_a,tendency_loc_cld,pvfa, &
...
!$acc present( &
!$acc   plude,pcovptot,prainfrac_toprfz) &
!$acc present( &
!$acc   pfsqlf,pfsqif,pfcqnng, &
!$acc   pfcqlng ,pfsqrf,pfsqsf,pfcqrng,pfcqsng,pfsqltur, &
!$acc   pfsqitur,pfplsl,pfplsn,pfhpsl,pfhpsn,tmp1,tmp2&
!$acc   )
...
!$ACC LOOP VECTOR &
!$ACC& PRIVATE(IPHASE,IMELT,LLFALL,ZQX,ZQXN,ZQXFG,ZQXNM1,ZSOLQA,ZSOLQB,ZQLHS,ZVQX,ZQXN2D,&
!$acc& ZFALLSINK,ZFALLSRCE,ZCONVSRCE,ZCONVSINK,ZPSUPSATSRCE,ZLNEG,ZPFPLSX,ZQX0)
      DO JL=1,ICEND
...
#define DEPICE 1
        ...
        LLFALL(NCLDQR)=ZVQX(NCLDQR)>0.0_JPRB
        LLFALL(NCLDQS)=ZVQX(NCLDQS)>0.0_JPRB
        ! water vapour and liquid cloud water does not fall
        LLFALL(NCLDQV)=.false.
        LLFALL(NCLDQL)=.false.
        ...
        !$acc loop seq
        DO JM=1,NCLV-1
          ZPFPLSX(JM)=0.0_JPRB ! precip fluxes at bottom level
        ENDDO
...
!$acc loop seq
        DO JK=1,KLEV
          ...
          ! ----------------------
          ZTP1=PT(JL,JK,IBL)+PTSPHY*TENDENCY_TMP_T(JL,JK,IBL)
          ZQX(NCLDQV)=PQ(JL,JK,IBL)+PTSPHY*TENDENCY_TMP_Q(JL,JK,IBL)
          ZA=PA(JL,JK,IBL)+PTSPHY*TENDENCY_TMP_A(JL,JK,IBL)
          ZAORIG=PA(JL,JK,IBL)+PTSPHY*TENDENCY_TMP_A(JL,JK,IBL)
...
!$acc loop seq
          DO JM=1,NCLV-1
            ZQX(JM)=PCLV(JL,JK,JM,IBL)+PTSPHY*TENDENCY_TMP_CLD(JL,JK,JM,IBL)
            ZQX0(JM)=ZQX(JM)
          ENDDO
...
!$acc loop seq
          DO JM=1,NCLV-1
            ZLNEG(JM)=0.0_JPRB ! zero array
          ENDDO

          ! ----------------------------------------------------
          ! Tidy up very small cloud cover or total cloud water
          ! ----------------------------------------------------
          IF(ZQX(NCLDQL)+ZQX(NCLDQI)<YRECLDP%RLMIN.or.ZA<YRECLDP%RAMIN) THEN
            ...
            TENDENCY_LOC_T_=TENDENCY_LOC_T_-RALVDCP*ZQADJ
            ...
            TENDENCY_LOC_Q_=TENDENCY_LOC_Q_+ZQADJ
            TENDENCY_LOC_T_=TENDENCY_LOC_T_-RALSDCP*ZQADJ
...
!$acc loop seq
          DO JM=1,NCLV-1
            ...
            IF(ZQX(JM)<YRECLDP%RLMIN) THEN
              ...
              ZQADJ=ZQX(JM)*ZQTMST
              TENDENCY_LOC_Q_=TENDENCY_LOC_Q_+ZQADJ
              IF(IPHASE(JM)==1) TENDENCY_LOC_T_=TENDENCY_LOC_T_-RALVDCP*ZQADJ
              IF(IPHASE(JM)==2) TENDENCY_LOC_T_=TENDENCY_LOC_T_-RALSDCP*ZQADJ
          ...
          ZFOEEWMT=MIN(FOEEWM(ZTP1)/PAP(JL,JK,IBL),0.5_JPRB)
          ZQSMIX=ZFOEEWMT
          ...
          !---------------------------------------------
          ZALFA=FOEDELTA(ZTP1)
          ZFOEEW=MIN((ZALFA*FOEELIQ(ZTP1)+(1.0_JPRB-ZALFA)*FOEEICE(ZTP1))/PAP(JL,JK,IBL),0.5_JPRB)
          ZQSICE=ZFOEEW/(1.0_JPRB-RETV*ZFOEEW)

          !----------------------------------
          ! liquid water saturation
          !----------------------------------
          ZFOEELIQT=MIN(FOEELIQ(ZTP1)/PAP(JL,JK,IBL),0.5_JPRB)
          ZQSLIQ=ZFOEELIQT
          ...
          !   ! ice water saturation
          !   !----------------------------------
          !   ZFOEEICET(JL,JK)=MIN(FOEEICE(ZTP1)/PAP(JL,JK),0.5_JPRB)
          !   ZQSICE=ZFOEEICET(JL,JK)
          !   ZQSICE=ZQSICE/(1.0_JPRB-RETV*ZQSICE)

          !------------------------------------------
          ! Ensure cloud fraction is between 0 and 1
          !------------------------------------------
          ZA=MAX(0.0_JPRB,MIN(1.0_JPRB,ZA))
...
!$acc loop seq
            ...
            ZGDP=RG/ZDP ! g/dp
            ZRHO=PAP(JL,JK,IBL)/((RD*ZTP1)) ! p/RT air density
            ...
            ZCORQSMIX=1.0_JPRB+FOELDCPM(ZTP1)*ZDQSMIXDT
            ...
            IF(ZTP1>=RTT.or.YRECLDP%NSSOPT==0) THEN
              ZFAC=1.0_JPRB
              ZFACI=1.0_JPRB
            ELSE
              ZFAC=ZA+ZFOKOOP*(1.0_JPRB-ZA)
            ...
            IF(ZA>1.0_JPRB-YRECLDP%RAMIN) THEN
              ZSUPSAT=MAX((ZQX(NCLDQV)-ZFAC*ZQSICE)/ZCORQSICE,0.0_JPRB)
            ELSE
              ! Calculate environmental humidity supersaturation
              ZQP1ENV=(ZQX(NCLDQV)-ZA*ZQSICE)/MAX(1.0_JPRB-ZA,ZEPSILON)
              !& SIGN(MAX(ABS(1.0_JPRB-ZA),ZEPSILON),1.0_JPRB-ZA)
              ZSUPSAT=MAX(((1.0_JPRB-ZA)*(ZQP1ENV-ZFAC*ZQSICE))/ZCORQSICE,0.0_JPRB)
            ENDIF
            ...
            IF(JK>YRECLDP%NCLDTOP) THEN
              ...
              ZDTDP=(ZRDCP*0.5_JPRB*(ZTP1_PREV+ZTP1))/PAPH(JL,JK,IBL)
...
!$acc loop seq
            ...
            ZDTDP=(ZRDCP*ZTP1)/PAP(JL,JK,IBL)
            ZDPMXDT=ZDP*ZQTMST
            ZMFDN=0.0_JPRB
            ...
            ZQSAT=ZQSAT*ZCOR
            ZCOND=(ZQSMIX-ZQSAT)/(1.0_JPRB+ZQSAT*ZCOR*FOEDEM(ZTP1))
            ZTP1=ZTP1+FOELDCPM(ZTP1)*ZCOND
            ZQSMIX=ZQSMIX-ZCOND
            ZQSAT=FOEEWM(ZTP1)*ZQP
            ZQSAT=MIN(0.5_JPRB,ZQSAT)
            ZCOR=1.0_JPRB/(1.0_JPRB-RETV*ZQSAT)
            ZQSAT=ZQSAT*ZCOR
            ZCOND1=(ZQSMIX-ZQSAT)/(1.0_JPRB+ZQSAT*ZCOR*FOEDEM(ZTP1))
            ...
            IF(ZDQS>0.0_JPRB) THEN
              ...
              !    ZLEVAP = MAX(ZA+ZACUST(JL),1.0_JPRB)*MIN(ZDQS(JL),ZLICLD(JL)+ZLFINALSUM(JL))
              ZLEVAP=ZA*MIN(ZDQS,ZLICLD)
            ...
            IF(ZDQS<=-YRECLDP%RLMIN.and.ZA<1.0_JPRB-ZEPSEC) THEN
              ...
              IF(YRECLDP%NSSOPT==0) THEN
                ! No scheme
                ZQE=(ZQX(NCLDQV)-ZA*ZQSICE)/MAX(ZEPSEC,1.0_JPRB-ZA)
              ...
              IF(ZQE>=ZRHC*ZQSICE*ZFAC.and.ZQE<ZQSICE*ZFAC) THEN
                ...
                ZACOND=-((1.0_JPRB-ZA)*ZFAC*ZDQS)/MAX(2.0_JPRB*(ZFAC*ZQSICE-ZQE),ZEPSEC)

                ZACOND=MIN(ZACOND,1.0_JPRB-ZA) !PUT THE LIMITER BACK
                ...
                ZZDL=(2.0_JPRB*(ZFAC*ZQSICE-ZQE))/MAX(ZEPSEC,1.0_JPRB-ZA)
                ! Added correction term ZFAC 15/03/2010
                IF(ZFAC*ZDQS<-ZZDL) THEN
                  ! ZLCONDLIM=(ZA-1.0_JPRB)*ZDQS(JL)-ZQSICE+ZQX(NCLDQV)
                  ZLCONDLIM=(ZA-1.0_JPRB)*ZFAC*ZDQS-ZFAC*ZQSICE+ZQX(NCLDQV)
                  ZLCOND2=MIN(ZLCOND2,ZLCONDLIM)
                ENDIF
                ZLCOND2=MAX(ZLCOND2,0.0_JPRB)

                IF(ZLCOND2<YRECLDP%RLMIN.or.(1.0_JPRB-ZA)<ZEPSEC) THEN
                  ZLCOND2=0.0_JPRB
                  ZACOND=0.0_JPRB
                ENDIF
            ...
            IF(IDEPICE==1) THEN
              ...
              IF(ZA_PREV<YRECLDP%RCLDTOPCF.and.ZA>=YRECLDP%RCLDTOPCF) THEN
                ZCLDTOPDIST=0.0_JPRB
              ELSE
              ...
              !--------------------------------------------------------------
              IF(ZTP1<RTT.and.ZQXFG(NCLDQL)>YRECLDP%RLMIN) THEN
                ! T<273K

                ZVPICE=(FOEEICE(ZTP1)*RV)/RD
                ...
                !------------------------------------------------
                ZADD=(RLSTT*(RLSTT/((RV*ZTP1))-1.0_JPRB))/((2.4E-2_JPRB*ZTP1))
                ZBDD=(RV*ZTP1*PAP(JL,JK,IBL))/((2.21_JPRB*ZVPICE))
                ...
                !---------------------------
                ZDEPOS=MAX(ZA*(ZINEW-ZICE0),0.0_JPRB)
            ...
            ELSE IF(IDEPICE==2) THEN
              ...
              IF(ZTP1<RTT.and.ZQXFG(NCLDQL)>YRECLDP%RLMIN) THEN
                ...
                ZAPLUSB=  &
                & YRECLDP%RCL_APB1*ZVPICE-YRECLDP%RCL_APB2*ZVPICE*ZTP1+PAP(JL,JK,IBL)*YRECLDP%RCL_APB3*ZTP1**3._JPRB
                ZCORRFAC=(1.0_JPRB/ZRHO)**0.5_JPRB
                ZCORRFAC2=((ZTP1/273.0_JPRB)**1.5_JPRB)*(393.0_JPRB/(ZTP1+120.0_JPRB))

                ZPR02=(ZRHO*ZICE0*YRECLDP%RCL_CONST1I)/((ZTCG*ZFACX1I))

                ZTERM1=((ZVPLIQ-ZVPICE)*ZTP1**2.0_JPRB*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2I*ZFACX1I)/  &
                & ((ZRHO*ZAPLUSB*ZVPICE))
                ...
                ZDEPOS=MAX(ZA*ZTERM1*ZTERM2*PTSPHY,0.0_JPRB)
...
!$acc loop seq
            DO JM=1,NCLV-1
              IF(LLFALL(JM).or.JM==NCLDQI) THEN
                ...
                !-------------------------------------------------
                ! ZFALL = ZFALL*((PAP(JL,JK, IBL)*RICEHI1)**(-0.178_JPRB)) &
                !            &*((ZTP1*RICEHI2)**(-0.394_JPRB))
            ...
            IF(ZQPRETOT>ZEPSEC) THEN
              ZCOVPTOT=1.0_JPRB-((1.0_JPRB-ZCOVPTOT)*(1.0_JPRB-MAX(ZA,ZA_PREV)))/(1.0_JPRB-MIN(ZA_PREV,1.0_JPRB-1.E-06_JPRB))
              ZCOVPTOT=MAX(ZCOVPTOT,YRECLDP%RCOVPMIN)
              ZCOVPCLR=MAX(0.0_JPRB,ZCOVPTOT-ZA) ! clear sky proportion
            ...
            IF(ZTP1<=RTT) THEN
              ...
              IF(ZICECLD>ZEPSEC) THEN
                ...
                ZZCO=PTSPHY*YRECLDP%RSNOWLIN1*EXP(YRECLDP%RSNOWLIN2*(ZTP1-RTT))
            ...
            IF(ZLIQCLD>ZEPSEC) THEN
              ...
              ELSE IF(IWARMRAIN==2) THEN
                ...
                IF(ZLIQCLD>ZLCRIT) THEN
                  ...
                  ZRAINAUT=1.5_JPRB*ZA*PTSPHY*YRECLDP%RCL_KKAAU*ZLIQCLD**YRECLDP%RCL_KKBAUQ*ZCONST**YRECLDP%RCL_KKBAUN

                  ZRAINAUT=MIN(ZRAINAUT,ZQXFG(NCLDQL))
                  IF(ZRAINAUT<ZEPSEC) ZRAINAUT=0.0_JPRB

                  ZRAINACC=2.0_JPRB*ZA*PTSPHY*YRECLDP%RCL_KKAAC*(ZLIQCLD*ZRAINCLD)**YRECLDP%RCL_KKBAC
            ...
            IF(IWARMRAIN>1) THEN
              ...
              IF(ZTP1<=RTT.and.ZLIQCLD>ZEPSEC) THEN
            ...
            IF(ZICETOT>ZEPSEC.and.ZTP1>RTT) THEN

              ! Calculate subsaturation
              ZSUBSAT=MAX(ZQSICE-ZQX(NCLDQV),0.0_JPRB)

              ! Calculate difference between dry-bulb (ZTP1, IBL) and the temperature
              ...
              ! due to evaporation.
              ZTDMTW0=ZTP1-RTT-ZSUBSAT*(ZTW1+ZTW2*(PAP(JL,JK,IBL)-ZTW3)-ZTW4*(ZTP1-ZTW5))
              ! Not implicit yet...
...
!$acc loop seq
            ...
            IF(ZQX(NCLDQR)>ZEPSEC) THEN
              ...
              IF(ZTP1<=RTT.and.ZTP1_PREV>RTT) THEN
              ...
              IF(ZTP1<RTT) THEN
                ...
                IF(PRAINFRAC_TOPRFZ(JL,IBL)>0.8) THEN
                  ...
                  ZTEMP=YRECLDP%RCL_FZRAB*(ZTP1-RTT)
                ...
                ELSE
                  ...
                  ZCONS1=ABS((PTSPHY*(1.0_JPRB+0.5_JPRB*(RTT-ZTP1)))/YRECLDP%RTAUMEL)
                  ZFRZMAX=MAX((RTT-ZTP1)*ZCONS1*ZRLDCP,0.0_JPRB)
            ...
            !----------------------------------------------------------------------
            ! not implicit yet...
            ZFRZMAX=MAX((YRECLDP%RTHOMO-ZTP1)*ZRLDCP,0.0_JPRB)
            ...
            IF(IEVAPRAIN==1) THEN
              ...
              ZZRH=YRECLDP%RPRECRHMAX+((1.0_JPRB-YRECLDP%RPRECRHMAX)*ZCOVPMAX)/MAX(ZEPSEC,1.0_JPRB-ZA)
              ZZRH=MIN(MAX(ZZRH,YRECLDP%RPRECRHMAX),1.0_JPRB)

              ZQE=(ZQX(NCLDQV)-ZA*ZQSLIQ)/MAX(ZEPSEC,1.0_JPRB-ZA)
              !---------------------------------------------
              ...
              IF(LLO1) THEN
                ...
                !-------------------------------------------------------------
                ZCOVPTOT=MAX(YRECLDP%RCOVPMIN,ZCOVPTOT-MAX(0.0_JPRB,((ZCOVPTOT-ZA)*ZEVAP)/ZQXFG(NCLDQR)))
            ...
            ELSE IF(IEVAPRAIN==2) THEN
              ...
              IF(LLO1) THEN
                ...
                ZESATLIQ=(RV/RD)*FOEELIQ(ZTP1)

                ! Slope of particle size distribution
                ZLAMBDA=(YRECLDP%RCL_FAC1/((ZRHO*ZPRECLR)))**YRECLDP%RCL_FAC2 ! ZPRECLR=kg/kg

                ZEVAP_DENOM=YRECLDP%RCL_CDENOM1*ZESATLIQ-YRECLDP%RCL_CDENOM2*ZTP1*ZESATLIQ&
                & +YRECLDP%RCL_CDENOM3*ZTP1**3._JPRB*PAP(JL,JK,IBL)

                ! Temperature dependent conductivity
                ZCORR2=((ZTP1/273._JPRB)**1.5_JPRB*393._JPRB)/(ZTP1+120._JPRB)

                ZSUBSAT=MAX(ZZRH*ZQSLIQ-ZQE,0.0_JPRB)

                ZBETA=(0.5_JPRB/ZQSLIQ)*ZTP1**2._JPRB*ZESATLIQ*YRECLDP%RCL_CONST1R*(ZCORR2/  &
            ...
            IF(IEVAPSNOW==1) THEN
              ...
              IF(LLO1) THEN
                ...
                ! 2-flux treatment, abandoned due to the new prognostic precip
                !-------------------------------------------------------------
                ZCOVPTOT=MAX(YRECLDP%RCOVPMIN,ZCOVPTOT-MAX(0.0_JPRB,((ZCOVPTOT-ZA)*ZEVAP)/ZQXFG(NCLDQS)))
            ...
            ELSE IF(IEVAPSNOW==2) THEN
              ...
              IF(LLO1) THEN
                ...
                ZTCG=1.0_JPRB !v1 EXP(RCL_X3I*(273.15_JPRB-ZTP1)/8.18_JPRB)
                ! ZFACX1I modification is based on Andrew Barrett's results
                ZFACX1S=1.0_JPRB !v1 (ZICE0/1.E-5_JPRB)**0.627_JPRB

                ZAPLUSB=  &
                & YRECLDP%RCL_APB1*ZVPICE-YRECLDP%RCL_APB2*ZVPICE*ZTP1+PAP(JL,JK,IBL)*YRECLDP%RCL_APB3*ZTP1**3
                ZCORRFAC=(1.0_JPRB/ZRHO)**0.5_JPRB
                ZCORRFAC2=((ZTP1/273.0_JPRB)**1.5_JPRB)*(393.0_JPRB/(ZTP1+120.0_JPRB))

                ZPR02=(ZRHO*ZPRECLR*YRECLDP%RCL_CONST1S)/((ZTCG*ZFACX1S))

                ZTERM1=((ZQSICE-ZQE)*ZTP1**2*ZVPICE*ZCORRFAC2*ZTCG*YRECLDP%RCL_CONST2S*ZFACX1S)/  &
                & ((ZRHO*ZAPLUSB*ZQSICE))
                ZTERM2=0.65_JPRB*YRECLDP%RCL_CONST6S*ZPR02**YRECLDP%RCL_CONST4S+  &
                & (YRECLDP%RCL_CONST3S*ZCORRFAC**0.5_JPRB*ZRHO**0.5_JPRB*ZPR02**YRECLDP%RCL_CONST5S)/ZCORRFAC2**0.5_JPRB
                ...
                ! 2-flux treatment, abandoned due to the new prognostic precip
                !-------------------------------------------------------------
                ZCOVPTOT=MAX(YRECLDP%RCOVPMIN,ZCOVPTOT-MAX(0.0_JPRB,((ZCOVPTOT-ZA)*ZEVAP)/ZQX(NCLDQS)))
...
!$acc loop seq
            ...
            !---------------------------
            ZANEW=(ZA+ZSOLAC)/(1.0_JPRB+ZSOLAB)
...
!$acc loop seq
            DO JM=1,NCLV-1
              ...
              ZFLUXQ=ZPSUPSATSRCE(JM)+ZCONVSRCE(JM)+ZFALLSRCE(JM)-(ZFALLSINK(JM)+ZCONVSINK(JM))*ZQXN(JM)

              IF(IPHASE(JM)==1) THEN
                TENDENCY_LOC_T_=TENDENCY_LOC_T_+RALVDCP*(ZQXN(JM)-ZQX(JM)-ZFLUXQ)*ZQTMST
              ENDIF

              IF(IPHASE(JM)==2) THEN
                TENDENCY_LOC_T_=TENDENCY_LOC_T_+RALSDCP*(ZQXN(JM)-ZQX(JM)-ZFLUXQ)*ZQTMST
              ENDIF
              ...
              !----------------------------------------------------------------------
              TENDENCY_LOC_CLD(JL,JK,JM,IBL)=(ZQXN(JM)-ZQX0(JM))*ZQTMST

            ENDDO
            TENDENCY_LOC_CLD(JL,JK,NCLV,IBL)=0.0_JPRB

            !----------------------
            ! 6.2 Humidity budget
            !----------------------
            TENDENCY_LOC_q_=TENDENCY_LOC_Q_+(ZQXN(NCLDQV)-ZQX(NCLDQV))*ZQTMST

            !-------------------
            ! 6.3 cloud cover
            !-----------------------
            TENDENCY_LOC_A_=TENDENCY_LOC_A_+ZDA*ZQTMST
...
!$acc loop seq
            DO JM=1,NCLV
              TENDENCY_LOC_CLD(JL,JK,JM,IBL)=0.0_JPRB
            ENDDO
!$acc loop seq
            DO JM=1,NCLV-1
              ZQXN2D(JM)=0.0_JPRB
...
!$acc loop seq
            DO JM=1,NCLV-1
              ZPFPLSX(JM)=0.0_JPRB ! precip fluxes at next level
            ENDDO

          ENDIF
          TENDENCY_LOC_T(JL,JK,IBL)=TENDENCY_LOC_T_
          TENDENCY_LOC_A(JL,JK,IBL)=TENDENCY_LOC_A_
          TENDENCY_LOC_Q(JL,JK,IBL)=TENDENCY_LOC_Q_
          ...
          PFSQLF(JL,JK+1,IBL)=PFSQLF(JL,JK+1,IBL)+(ZQXN2D(NCLDQL)-ZQX0(NCLDQL)+PVFL(JL,JK,IBL)*PTSPHY&
             & -ZALFAW*PLUDE(JL,JK,IBL))*ZGDPH_R
          ...
          PFSQIF(JL,JK+1,IBL)=PFSQIF(JL,JK+1,IBL)+(ZQXN2D(NCLDQI)-ZQX0(NCLDQI)+PVFI(JL,JK,IBL)*PTSPHY&
  & -(1.0_JPRB-ZALFAW)*PLUDE(JL,JK,IBL))*ZGDPH_R
          ...
          !--------------------------------------------------------------------
          PFPLSL(JL,JK+1,IBL)=ZPFPLSX(NCLDQR)+ZPFPLSX(NCLDQL)
          PFPLSN(JL,JK+1,IBL)=ZPFPLSX(NCLDQS)+ZPFPLSX(NCLDQI)

          !-----------------------------------
          ! enthalpy flux due to precipitation
          !-----------------------------------
          PFHPSL(JL,JK+1,IBL)=-RLVTT*PFPLSL(JL,JK+1,IBL)
          PFHPSN(JL,JK+1,IBL)=-RLSTT*PFPLSN(JL,JK+1,IBL)
...
!$acc end data
  ...
  ENDSUBROUTINE CLOUDSC_SCC_OPT
ENDMODULE CLOUDSC_GPU_SCC_OPT_MOD
