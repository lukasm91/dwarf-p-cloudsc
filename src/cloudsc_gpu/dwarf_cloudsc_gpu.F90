...
#ifdef CLOUDSC_GPU_SCC_OPT
USE CLOUDSC_DRIVER_GPU_SCC_OPT_MOD, ONLY: CLOUDSC_DRIVER_GPU_SCC_OPT
#endif
...
CHARACTER(LEN=255) :: PERFTEST
LOGICAL LPERFTEST

#include "abor1.intfb.h"

CALL GET_ENVIRONMENT_VARIABLE("PERFTEST", PERFTEST)
LPERFTEST = .false.
IF (TRIM(PERFTEST) == "1") LPERFTEST=.true.
...
#ifdef CLOUDSC_GPU_SCC_OPT
print '(1X,A42)', 'Executing CLOUDSC-GPU, "SCC-opt" variant...'

 ! Call the driver to perform the parallel loop over our kernel
CALL CLOUDSC_DRIVER_GPU_SCC_OPT(NUMOMP, NPROMA, GLOBAL_STATE%KLEV, NGPTOT, GLOBAL_STATE%NBLOCKS, NGPTOTG, &
     & GLOBAL_STATE%KFLDX, GLOBAL_STATE%PTSPHY, &
     & GLOBAL_STATE%PT, GLOBAL_STATE%PQ, &
     & GLOBAL_STATE%B_CML,   GLOBAL_STATE%B_TMP, GLOBAL_STATE%B_LOC, &
     & GLOBAL_STATE%PVFA,    GLOBAL_STATE%PVFL,  GLOBAL_STATE%PVFI, &
