...
CONTAINS
  ...
  SUBROUTINE CLOUDSC_DRIVER( &
    ...
    ALLOCATE(BUFFER_TMP(NPROMA,NLEV,3+NCLV,NGPBLKS))
    ALLOCATE(BUFFER_LOC(NPROMA,NLEV,3+NCLV,NGPBLKS))
1003 format(5x,'NUMPROC=',i0,', NUMOMP=',i0,', NGPTOTG=',i0,', NPROMA=',i0,', NGPBLKS=',i0)
    ...
    DO JKGLO=1,NGPTOT,NPROMA
      ...
      OPEN(10, FILE="serialized_fortran.dat", status='replace', form='unformatted')
      ...
      DO IBL=1,NGPBLKS
       BUFFER_TMP(:,:,1,IBL) = TENDENCY_TMP(IBL)%T
       BUFFER_TMP(:,:,3,IBL) = TENDENCY_TMP(IBL)%Q(:,:)
       BUFFER_TMP(:,:,2,IBL) = TENDENCY_TMP(IBL)%A(:,:)
       BUFFER_TMP(:,:,4:,IBL) = TENDENCY_TMP(IBL)%CLD(:,:,:)
       BUFFER_LOC(:,:,1,IBL) = TENDENCY_LOC(IBL)%T(:,:)
       BUFFER_LOC(:,:,3,IBL) = TENDENCY_LOC(IBL)%Q(:,:)
       BUFFER_LOC(:,:,2,IBL) = TENDENCY_LOC(IBL)%A(:,:)
       BUFFER_LOC(:,:,4:,IBL) = TENDENCY_LOC(IBL)%CLD(:,:,:)
      ENDDO
